...
class QuestionTemplate():
    def __init__(self):
        self.q_template_dict={    
            0:self.get_movie_introduction,
            1:self.get_movie_type,
            2:self.get_movie_rating,
            3:self.get_movie_actor_list,
            4:self.get_movie_releasedate,
            5:self.get_actor_info,
            6:self.get_cooperation_movie_list,
            7:self.get_actor_movie_num,
            8:self.get_actor_act_movie_list,
            9:self.get_movie_rating_bigger,
            10:self.get_actor_act_type_movie              
        }

        # 连接数据库
        self.graph = Query()
        # 测试数据库是否连接上
        # result=self.graph.run("match (m:Movie)-[]->() where m.title='霸王别姬' return m.rating")
        # print(result)
        # exit()
